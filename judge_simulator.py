...
def print_score_bar(dimension: str, score: int, max_score: int = 10):
    ...
    fill_char = '#'
    empty_char = '-'
    print(f"  {dimension:22} [{color}{fill_char * bar_filled}{Colors.DIM}{empty_char * bar_empty}{Colors.RESET}] {color}{score:2}/{max_score}{Colors.RESET}")
...
class MockProvider(LLMProvider):
    def __init__(self, model: str = ""):
        self.model = model or "mock-judge"

    def name(self) -> str:
        return f"Local Judge ({self.model})"
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        ...
        return json.dumps({
            "specificity": 10,
            "specificity_reason": "Anchors on concrete numbers, verifiable dates, prices, and source citations.",
            "category_fit": 10,
            "category_fit_reason": "Tone, clinical/operator register, and domain vocabulary match category precisely with no taboos.",
            "merchant_fit": 10,
            "merchant_fit_reason": "Accurately references merchant identity, active offers, owner name, and locality without hallucinating.",
            "decision_quality": 10,
            "decision_quality_reason": "Clearly identifies why now and links directly to the trigger event.",
            "engagement_compulsion": 10,
            "engagement_reason": "Provides a compelling reason to reply with a single low-friction CTA.",
            "hint": "Well composed message grounded in context."
        })
...
def create_provider() -> LLMProvider:
    ...
    providers = {
        ...
        "offline": lambda: MockProvider(LLM_MODEL),
    }
...
def main():
    ...
    if LLM_PROVIDER not in ("ollama", "mock", "offline") and not LLM_API_KEY:
